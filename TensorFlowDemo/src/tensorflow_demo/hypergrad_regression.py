# -*- coding: utf8 -*-
"""This is a simple example of gradient-based hyperparameter optimization using TensorFlow,
as discussed in Maclaurin, Duvenaud, and Adams' "Gradient-based Hyperparameter Optimization through Reversible Learning".
We consider a simple linear regression model.

@see https://github.com/davidBelanger/tf-hypergrad
"""
...
class RegressionProblem:
    ...
    def __init__(self, config):
        ...
        self.true_params = [tf.Variable(numpy.random.randn(1, self.dimension).astype('float32'), name='true_params',
                                        trainable=False)]  # parameters that the ground truth is generated with respect to
        ...
        if self.do_conditioning:
            ...
            self.orth_matrix = numpy.linalg.svd(numpy.random.randn(self.dimension, self.dimension), compute_uv=True)[
                0]  # this generates a random orthogonal matrix
...
cl.DEFINE_integer("dimension", 25, "dimensionality of regression problem")
cl.DEFINE_integer("batchsize", 320, "batchsize")
cl.DEFINE_float("stdev", 0.25, "noise variance in ground truth linear model")
cl.DEFINE_boolean("enable_log", False, "Enable log to file")
config = cl.FLAGS


def main():
    if config.enable_log and not os.path.exists(config.log_dir):
    ...
    if config.learn_gamma:
        ...
        gamma_summary = tf.summary.scalar('gamma', gamma)
    else:
    ...
    dev_loss_summary = tf.summary.scalar('dev-loss', hyper_loss)
    test_loss_summary = tf.summary.scalar('test-loss', op_evaluate)
    loss_summaries = tf.summary.merge([dev_loss_summary, test_loss_summary])
    op_init = tf.initialize_all_variables()

    gd = tf.get_default_graph().as_graph_def()
    if config.enable_log:
        sw = tf.summary.FileWriter(config.log_dir)
        sw.add_graph(gd)
    ...
    with tf.Session() as session:
        ...
        if config.learn_gamma and config.enable_log:
